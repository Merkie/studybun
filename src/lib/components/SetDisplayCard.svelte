...
<div class="main">
	...
	<span class="subtext">
		<img class="profile-img" src={author_image} alt="" /><a
			style="color: var(--text-color); text-decoration: none;"
			href={`/profile/${author_id}`}>{author_name}</a
		>
	</span>

	<span style="font-weight: normal; font-size: 16px; margin-top: 5px; opacity: .5;"
		>Updated {format(created_at)}</span
	>
	{#if edit}
